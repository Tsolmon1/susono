...
class Tank_auth
{
	...
	function __construct()
	{
		...
		$this->ci->load->model('tank_auth/admins');
	...
	function login_admin($login, $password)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user = $this->ci->admins->get_user_by_login($login))) {	// login ok

				// Does password match hash in database?
				//$hasher = new PasswordHash(
				//		$this->ci->config->item('phpass_hash_strength', 'tank_auth'),
				//		$this->ci->config->item('phpass_hash_portable', 'tank_auth'));
				if (!is_null($user = $this->ci->admins->get_user_by_pass(base64_encode($password)))) {		// password ok
			
					$contractid = $this->ci->admins->get_user_id($user->adminname);
						if ($contractid == null) {
							$this->error = array('login_admin' => "Буруу хэрэглэгч");
						} else {
							$this->ci->session->set_userdata(array(
									...
									'admin_contract_id' => $contractid
							));			
							
								//$this->clear_login_attempts($login);
			
								/*$this->ci->admins->update_login_info(
										$user->id,
					...
					$this->error = array('h-password_admin' => 'auth_incorrect_password');
				}
			} else {															// fail - wrong login
				//$this->increase_login_attempt($login);
				$this->error = array('login_admin' => 'auth_incorrect_login');
			}
			}
			return FALSE;
	}
	...
	function logout_admin()
	{
		...
		$this->ci->session->set_userdata(array('admin_id' => '', 'adminname' => '', 'permission' => '', 'admin_status' => '', 'admin_contract_id' => ''));
	...
	{
		return $this->ci->session->userdata('admin_contract_id');
	}
	...
	{
		return $this->ci->session->userdata('adminname');
	}
	
	function get_permission()
	{
		return $this->ci->session->userdata('permission');
	}
	...
	private function create_autologin($user_id)
	{
		...
		if ($this->ci->user_autologin->set($user_id, md5($key))) {
			set_cookie(array(
					...
					'value'		=> serialize(array('admin_id' => $user_id, 'key' => $key)),
	...
	private function delete_autologin()
	{
		...
		if ($cookie = get_cookie($this->ci->config->item('autologin_cookie_name', 'tank_auth'), TRUE)) {
			...
			$this->ci->load->model('tank_auth/user_autologin');
			$this->ci->user_autologin->delete($data['admin_id'], md5($data['key']));
	...
	private function autologin()
	{
		if (!$this->is_logged_in() AND !$this->is_logged_in(FALSE)) {			// not logged in (as any user)
			...
			if ($cookie = get_cookie($this->ci->config->item('autologin_cookie_name', 'tank_auth'), TRUE)) {
				...
				if (isset($data['key']) AND isset($data['admin_id'])) {

					$this->ci->load->model('tank_auth/user_autologin');
					if (!is_null($user = $this->ci->user_autologin->get($data['admin_id'], md5($data['key'])))) {

						// Login user
						$this->ci->session->set_userdata(array(
								'admin_id'	=> $user->id,
								'adminname'	=> $user->adminname,
								'admin_status'	=> STATUS_ACTIVATED,
						));
