...
class Tank_auth
{
	...
	function login($login, $password)
	{
		if ((strlen($login) > 0) AND (strlen($password) > 0)) {
			...
			if (!is_null($user2 = $this->ci->admins->get_user_by_login($login))) {	// login ok
					//echo $user2;
				...
				if (!is_null($user = $this->ci->admins->get_user_by_pass(base64_encode($password), $user2->username))) {		// password ok
			
					$contractid = $this->ci->admins->get_user_id($user->username);
						if ($contractid == null) {
							$this->error = array('name' => "Буруу хэрэглэгч");
						} else {
					...
					$this->error = array('pass' => 'auth_incorrect_password');
				}
			} else {															// fail - wrong login
				//$this->increase_login_attempt($login);
				$this->error = array('name' => 'auth_incorrect_login');
			}
			}
			return FALSE;
	}
	...
	function logout()
	{
		...
		$this->ci->session->set_userdata(array('id' => '', 'permission' => '', 'status' => '', 'username' => '', 'email' => '', 'phone' => '', 'company_id' => ''));		
	...
	{
		if($this->ci->session->userdata('status') == 1) return true;
		else return false;
	...
	function create_user($username, $email, $password, $email_activation)
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available($username)) {
			$this->error = array('registerno' => 'auth_username_in_use');
	...
	function delete_user($password)
	{
		...
		if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
			...
			if ($hasher->CheckPassword($password, $user->password)) {			// success
				...
				$this->error = array('rwd' => 'auth_incorrect_password');
			}
		}
		return FALSE;
	}
	...
	private function create_autologin($user_id)
	{
		...
		if ($this->ci->user_autologin->set($user_id, md5($key))) {
			set_cookie(array(
					...
					'value'		=> serialize(array('id' => $user_id, 'key' => $key)),
	...
	private function delete_autologin()
	{
		...
		if ($cookie = get_cookie($this->ci->config->item('autologin_cookie_name', 'tank_auth'), TRUE)) {
			...
			$this->ci->load->model('tank_auth/user_autologin');
			$this->ci->user_autologin->delete($data['id'], md5($data['key']));
	...
	private function autologin()
	{
		if (!$this->is_logged_in() AND !$this->is_logged_in(FALSE)) {			// not logged in (as any user)
			...
			if ($cookie = get_cookie($this->ci->config->item('autologin_cookie_name', 'tank_auth'), TRUE)) {
				...
				if (isset($data['key']) AND isset($data['id'])) {

					$this->ci->load->model('tank_auth/user_autologin');
					if (!is_null($user = $this->ci->user_autologin->get($data['id'], md5($data['key'])))) {
